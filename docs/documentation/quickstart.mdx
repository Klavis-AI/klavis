          ...
          <Tab title="LlamaIndex">
           <CodeGroup>
              ...
              async def main():
                  ...
                  # Step 3: Connect LlamaIndex Agent with MCP Tools
                  mcp_client = BasicMCPClient(response.strata_server_url)
                  ...
                  # Step 5: Invoke the agent
                  handler = agent.run(
                      user_msg=f"summarize this video - https://youtu.be/yebNIHKAC4A?si=1Rz_ZsiVRz0YfOR7 and mail this summary to my email {my_email}"
                  )
          ...
          <Tab title="LlamaIndex">
            <CodeGroup>
              ...
              async def main() -> None:
                  ...
                  # Step 1: Create a single MCP server (example: Gmail)
                  response = klavis_client.mcp_server.create_server_instance(
                      server_name=McpServerName.GMAIL,
                      user_id="demo_user",
                  )
                  ...
                  # Step 3: Connect LlamaIndex Agent with MCP Tools
                  mcp_client = BasicMCPClient(response.server_url)
                  ...
                  # Step 4: Create a ReAct agent with MCP tools
                  agent = ReActAgent(
                      tools=tools,
                      llm=llm,
                      system_prompt="You are a helpful assistant that can use MCP tools."
                  ...
                  # Step 5: Invoke the agent
                  handler = agent.run(
                      user_msg="Search my inbox for unread emails and summarize."
                  )
                  result = await handler
