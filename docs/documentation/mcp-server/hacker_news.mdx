            ...
            async def main():
                ...
                mcp_client = MultiServerMCPClient({
                    "hacker_news": {
                        ...
                        "url": hacker_news_server.server_url
                    }
                })
                ...
                agent = create_react_agent(
                    model=llm,
                    tools=tools,
                    prompt="You are a tech news assistant that helps analyze and summarize Hacker News stories, trends, and discussions."
                )
                
                # Use the agent
                result = await agent.ainvoke({
                    "messages": [{"role": "user", "content": "What are the top 5 stories on Hacker News right now?"}]
                })
