            ...
            async def main():
                ...
                mcp_client = MultiServerMCPClient({
                    "doc2markdown": {
                        ...
                        "url": doc2markdown_server.server_url
                    }
                })
                ...
                agent = create_react_agent(
                    model=llm,
                    tools=tools,
                    prompt="You are a document conversion assistant that helps convert various document formats to markdown."
                )
                
                # Use the agent
                result = await agent.ainvoke({
                    "messages": [{"role": "user", "content": "Convert this PDF document to markdown format"}]
                })
