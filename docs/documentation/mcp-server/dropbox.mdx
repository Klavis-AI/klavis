          ...
          <Tab title="LangChain">
            ...
            async def main():
                ...
                agent = create_react_agent(
                    model=llm,
                    tools=tools,
                    prompt="You are a file management assistant that helps organize and manage Dropbox files."
                )
                
                # Use the agent
                result = await agent.ainvoke({
                    "messages": [{"role": "user", "content": "List files in my root folder"}]
                })
