          ...
          <Tab title="LangChain">
            ...
            async def main():
                ...
                agent = create_react_agent(
                    model=llm,
                    tools=tools,
                    prompt="You are a design assistant that helps access Figma files, manage assets, and collaborate on design projects."
                )
                
                # Use the agent
                result = await agent.ainvoke({
                    "messages": [{"role": "user", "content": "Get the latest comments from my design file"}]
                })
