          ...
          <Tab title="LangChain">
            ...
            async def main():
                ...
                agent = create_react_agent(
                    model=llm,
                    tools=tools,
                    prompt="You are a sales intelligence assistant that helps analyze sales conversations and call data."
                )
                
                # Use the agent
                result = await agent.ainvoke({
                    "messages": [{"role": "user", "content": "Get recent call transcripts for sales analysis"}]
                })
