            ...
            async def main():
                ...
                mcp_client = MultiServerMCPClient({
                    "postgres": {
                        ...
                        "url": postgres_server.server_url
                    }
                })
                ...
                agent = create_react_agent(
                    model=llm,
                    tools=tools,
                    prompt="You are a database analyst that helps with PostgreSQL queries, data analysis, and database management."
                )
                
                # Use the agent
                result = await agent.ainvoke({
                    "messages": [{"role": "user", "content": "Show me the top 10 customers by total orders"}]
                })
