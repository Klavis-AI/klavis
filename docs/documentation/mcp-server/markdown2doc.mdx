            ...
            async def main():
                ...
                mcp_client = MultiServerMCPClient({
                    "markdown2doc": {
                        ...
                        "url": markdown2doc_server.server_url
                    }
                })
                ...
                agent = create_react_agent(
                    model=llm,
                    tools=tools,
                    prompt="You are a document conversion assistant that helps convert markdown to various document formats."
                )
                
                # Use the agent
                result = await agent.ainvoke({
                    "messages": [{"role": "user", "content": "Convert this markdown to a PDF document"}]
                })
