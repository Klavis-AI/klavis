          ...
          <Tab title="LangChain">
            ...
            async def main():
                ...
                agent = create_react_agent(
                    model=llm,
                    tools=tools,
                    prompt="You are a project management assistant that helps manage Asana tasks."
                )
                
                # Use the agent
                result = await agent.ainvoke({
                    "messages": [{"role": "user", "content": "List all tasks in my current sprint"}]
                })
