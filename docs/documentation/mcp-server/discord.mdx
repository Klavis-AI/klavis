          ...
          <Tab title="LangChain">
            ...
            async def main():
                ...
                agent = create_react_agent(
                    model=llm,
                    tools=tools,
                    prompt="You are a Discord community assistant that helps manage servers and communication."
                )
                
                # Use the agent
                result = await agent.ainvoke({
                    "messages": [{"role": "user", "content": "Send a welcome message to the general channel"}]
                })
