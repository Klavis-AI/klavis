"""Stock split helpers backed by yfinance."""
...
async def fetch_splits(
    ...
    *,
    ...
    series = await anyio.to_thread.run_sync(_load_splits, symbol.upper())
    if series is None or series.empty:
        raise YahooFinanceError(f"No split data returned for '{symbol.upper()}'")

    windowed = _apply_window(series, period_start, period_end)
    if windowed.empty:
        raise YahooFinanceError("No splits found in the requested window")

    records = _series_to_records(windowed, value_key="ratio")
...
def _load_splits(symbol: str) -> pd.Series:
    ticker = yf.Ticker(symbol)
    data = getattr(ticker, "splits", None)
    if data is None:
