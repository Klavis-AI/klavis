...
def _capability_probe() -> Dict[str, Any]:
    ...
    try:
        ...
        req = svc.spaces().list(pageSize=1)
        resp = req.execute()
...
async def list_meetings(max_results: int = 10) -> Dict[str, Any]:
    ...
    try:
        ...
        while len(spaces) < max_results:
            kwargs = {"pageSize": min(50, max_results - len(spaces))}
            if page_token:
...
async def get_participants(space_id: str, max_results: int = 50) -> Dict[str, Any]:
    ...
    try:
        ...
        while len(participants) < max_results:
            kwargs = {"parent": parent, "pageSize": min(50, max_results - len(participants))}
            if page_token:
                kwargs["pageToken"] = page_token
            resp = svc.spaces().participants().list(**kwargs).execute()
