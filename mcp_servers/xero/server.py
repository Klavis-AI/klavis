...
def main(
            ...
            types.Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        "quote_id": {
                            "type": "string",
                            "description": "Quote ID to update (required)"
                        },
                        "contact_id": {
                            "type": "string",
            ...
            try:
                ...
                if not quote_id:
                    return [
                        types.TextContent(
                            type="text",
                            text="Error: 'quote_id' parameter is required for updating a quote",
                        )
                    ]
                
                # Extract optional parameters
                result = await xero_update_quote(
                    quote_id=quote_id,
                    contact_id=arguments.get("contact_id"),
                    date=arguments.get("date"),
                    expiry_date=arguments.get("expiry_date"),
    ...
    async def handle_sse(request):
        ...
        token = auth_token_context.set(xero_token or "")
        try:
